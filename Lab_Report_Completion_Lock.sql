-- Apply after Lab_Report_Save_Access.sql. Lock completed RSTs without deleting records or adding foreign keys.
SET ANSI_NULLS ON;
GO
...
BEGIN
 ...
 BEGIN TRANSACTION;
 DECLARE @LockResult int, @LockName nvarchar(255)=N'LabReport:'+UPPER(LTRIM(RTRIM(@RSTNumber)));
 EXEC @LockResult=sys.sp_getapplock @Resource=@LockName,@LockMode='Exclusive',@LockOwner='Transaction',@LockTimeout=10000;
 IF @LockResult<0 THROW 50001,'Another report is being saved for this RST. Please retry.',1;
 ...
 -- Match dashboard coverage, including unmapped items; same-submission retries return above.
 ...
 IF EXISTS(SELECT 1 FROM @Coverage WHERE RSTNumber=@RSTNumber AND ItemCount>0 AND UnmappedItems=0 AND RequiredTests>0 AND CompletedTests>=RequiredTests)
     THROW 50002,'All item tests for this RST are already submitted. Entry is closed; view the saved reports.',1;
