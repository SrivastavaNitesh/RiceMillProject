(() => {
    ...
    function renderResults() {
        ...
        for (const item of items) {
            const card = node('section', undefined, 'card card-body mb-3');
            card.append(node('h5', `${item.CategoryName} / ${item.ItemName}`));
            ...
            for (const test of item.Tests) {
                const row = node('div', undefined, 'row g-2 align-items-center mb-3');
                const labelCol = node('div', undefined, 'col-md-5');
                ...
                const inputCol = node('div', undefined, 'col-md-7');
                ...
                group.append(input);
                if (test.Unit) group.append(node('span', test.Unit, 'input-group-text'));
                ...
                row.append(labelCol, inputCol); card.append(row); index++;
            }
    ...
    function renderItems() {
        ...
        document.getElementById('category-menu').textContent = checked.size ? `${checked.size} category / categories selected` : 'Select categories';
